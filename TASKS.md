...
- [x] Compile-time reactive `let` (auto UI updates without manual `setText` / `refresh`) — Phase 1: `ui.state` + `ui.auto` (in-place props sync); Phase 2: `@close-by/clay-compiler` + `clay` Bun loader with non-leading / nested-block `let`s (see [`docs/reactive-let.md`](./docs/reactive-let.md)); NiceGUI full parity still Later
