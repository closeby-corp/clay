...
export function generateToastHTML(props: ToastProps): string {
  const positionClass = props.position || 'bottom-right';
  const alertClass = `alert alert-${props.type || 'info'}`;
  const duration = props.duration || 3000;

  return `
    <div class="toast toast-${positionClass} z-50"
         _="init wait ${duration}ms then remove me">
      <div class="${alertClass}">
        ${getToastIcon(props.type || 'info')}
